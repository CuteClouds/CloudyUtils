...
public abstract class Lexer<T> {
    ...
    protected void ensureHistoryCapacity(int size) {
        if (history.length > size) return;

        Entry[] h = history;
        history = new Entry[size + 2]; //just for sure lol
        System.arraycopy(h, 0, history, 0, h.length);
    }
